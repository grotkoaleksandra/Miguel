...
export function ContentPanel({ dict }: { dict: Dictionary }) {
  ...
  return (
    ...
    >
      {/* Backdrop */}
      <div className="absolute inset-0 bg-black/50 backdrop-blur-xl animate-fade-in" />
      ...
      >
        ...
        <button
          ...
          className="absolute top-0 right-0 text-white/30 hover:text-white/60 text-[10px] tracking-[0.2em] uppercase transition-colors duration-300"
          data-cursor-hover
        >
          Close
        </button>
...
function WorkContent({ dict }: { dict: Dictionary }) {
  ...
  return (
    <div>
      <h2 className="font-display text-4xl md:text-5xl font-normal tracking-[-0.02em] text-white mb-12">
        Selected Work
      </h2>
        ...
        {projects.map((p, i) => (
          <div key={p.title} className="group cursor-pointer" data-cursor-hover>
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                <h3 className="text-white text-lg font-light mb-1">{p.title}</h3>
                <span className="text-white/25 text-[10px] tracking-[0.2em] uppercase">{p.category}</span>
              </div>
              <span className="text-white/10 text-3xl font-display">{String(i + 1).padStart(2, "0")}</span>
            </div>
          </div>
        ))}
      </div>
...
function ServicesContent({ dict }: { dict: Dictionary }) {
  return (
    <div>
      <h2 className="font-display text-4xl md:text-5xl font-normal tracking-[-0.02em] text-white mb-4">
        What we do
      </h2>
      ...
      <div className="space-y-0">
        {dict.home.services.list.map((s, i) => (
          <div key={s.title} className="group border-t border-white/[0.06] py-6 flex items-center justify-between cursor-pointer" data-cursor-hover>
            <div className="flex items-center gap-6">
              <span className="text-white/10 text-[11px] font-mono w-6">{String(i + 1).padStart(2, "0")}</span>
              <h3 className="text-lg font-light group-hover:text-white/60 group-hover:translate-x-2 transition-all duration-500">
                {s.title}
              </h3>
            </div>
...
function AboutContent({ dict }: { dict: Dictionary }) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
      <div>
        <h2 className="font-display text-4xl md:text-5xl font-normal tracking-[-0.02em] text-white mb-8">
      ...
      <div className="flex flex-col justify-end">
        <div className="space-y-6">
          <div>
            <span className="text-white/15 text-[9px] tracking-[0.3em] uppercase block mb-2">Location</span>
            <span className="text-white/50 text-sm">Warsaw, Poland</span>
          </div>
          <div>
            <span className="text-white/15 text-[9px] tracking-[0.3em] uppercase block mb-2">Contact</span>
            <a href="mailto:hello@syrenacreative.com" className="text-white/50 text-sm link-hover hover:text-white/80 transition-colors" data-cursor-hover>
              hello@syrenacreative.com
            </a>
          </div>
          <div>
            <span className="text-white/15 text-[9px] tracking-[0.3em] uppercase block mb-2">Founded</span>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
      <div>
        ...
        <a
          ...
          className="inline-flex items-center gap-3 text-[11px] tracking-[0.2em] uppercase border border-white/15 rounded-full px-10 py-4 text-white/60 hover:bg-white hover:text-[#060a10] transition-all duration-500"
          data-cursor-hover
        >
      ...
      <div className="flex flex-col justify-end">
        <div className="space-y-6">
          <div>
            <span className="text-white/15 text-[9px] tracking-[0.3em] uppercase block mb-2">Email</span>
            <a href="mailto:hello@syrenacreative.com" className="text-white/50 text-sm" data-cursor-hover>
              hello@syrenacreative.com
            </a>
          </div>
          <div>
            <span className="text-white/15 text-[9px] tracking-[0.3em] uppercase block mb-2">Based in</span>
            <span className="text-white/50 text-sm">Warsaw, Poland</span>
          </div>
        </div>
      </div>
